...
class SideRetailBar extends React.Component {
	...
	render() {
		const menuVis = this.state.isactive ? 'show' : 'hide';
		const buttonType = this.state.isactive ? 'openNav' : 'closeNav';
		return (
			<>
			<button className={`${buttonType}`} isactive={this.state.isActive} onClick={this.toggleButton}>
				{this.state.isactive ? <AiFillCaretRight/> : <AiFillCaretLeft/>}
			</button>
			<div className={`retailPriceSidebar ${menuVis}`}>
				<div className="retailHeading">
					<h1 className="retailTitle">Retail Prices</h1>
				</div>
				<div className="retailers" style={{ overflow: "scroll"}}>
					<RetailProductCard
